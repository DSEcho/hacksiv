...
static bool MainLoop(bool retryCreate)
{
    ...
    // Create camera
    mainCam = new Camera(XMVectorSet(1.0f, 18.0f, 18.0f, 0), XMQuaternionIdentity());
	mainCam->Rot = XMQuaternionRotationRollPitchYaw(0, -1.15f, 0);
    ...
    while (DIRECTX.HandleMessages())
    {
        ...
        if (sessionStatus.IsVisible)
        {
			...
			if (DIRECTX.Key['A'])                            mainCam->Pos = XMVectorSubtract(mainCam->Pos, up);
			// TODO: set direction based on keyboard input
            static float Yaw = 0;
			...
			// MAIN SNAKE BLOCK
			static const int delay = 25; // delay between moves
			static int snakeClock = -1000; // want to immediatly draw when the game begins
			static float lastPos[3] = { 2.0f, 2.0f, 2.0f }; // last position of the head
			static float adjustment[3] = { 1.0f, 0, 0 }; // where from the last position are we moving, i.e. direction
			static Snek snake;

			// KEY INPUT
			if (DIRECTX.Key['J'] || DIRECTX.Key[VK_NUMPAD5] && check != 2) {//-X
				check = 1;
				adjustment[0] = -1.0f;
				adjustment[1] = 0;
				adjustment[2] = 0;
			}
			else if (DIRECTX.Key['L'] || DIRECTX.Key[VK_NUMPAD8] && check != 1)  { //+X
				check = 2;
				adjustment[0] = 1.0f;
				adjustment[1] = 0;
				adjustment[2] = 0;
			}
			else if (DIRECTX.Key['K'] || DIRECTX.Key[VK_NUMPAD6] && check != 4)  {//+Y
				check = 3;
				adjustment[0] = 0;
				adjustment[1] = 0;
				adjustment[2] = 1.0f;
			}
			else if (DIRECTX.Key['I'] || DIRECTX.Key[VK_NUMPAD4] && check != 3) { //-Y
				check = 4;
				adjustment[0] = 0;
				adjustment[1] = 0;
				adjustment[2] = -1.0f;
			}
			else if ((DIRECTX.Key['R'] || DIRECTX.Key[VK_NUMPAD9])&& check!=6)  {//Z
				check = 5;
				adjustment[0] = 0;
				adjustment[1] = 1.0f;
				adjustment[2] = 0;
			}
			else if ((DIRECTX.Key['F'] || DIRECTX.Key[VK_NUMPAD3] || DIRECTX.Key[VK_NUMPAD7])&&check !=5) {//-Z
				check = 6;
				adjustment[0] = 0;
				adjustment[1] = -1.0f;
				adjustment[2] = 0;
			...
			if (snakeClock == delay) {
				...
				int *pos = (snake.getFood()).getPos();
				food.AddSolidColorBox((float)pos[0], (float)pos[1], (float)pos[2], (float)pos[0] + 1, (float)pos[1] + 1, (float)pos[2] + 1, 0xffff0000);
				food.AddSolidColorBox((float)pos[0], 0.0f, (float)pos[2], (float)pos[0] + 1, 0.1f, (float)pos[2] + 1, 0xff000000);


				roomScene->Models[3] = new Model(&food, XMFLOAT3(0, 0, 0), XMFLOAT4(0, 0, 0, 1),
					new Material(
				...
				lastPos[0] += adjustment[0];
				lastPos[1] += adjustment[1];
				lastPos[2] += adjustment[2];


				snake.move(new int[3]{(int)lastPos[0], (int)lastPos[1], (int)lastPos[2] });

				TriangleSet snake_set;
				Node* current = snake.getHead();
				while(current != nullptr) {
					int* pos_snake = current->getItem();
					snake_set.AddSolidColorBox((float)pos_snake[0], (float)pos_snake[1], (float)pos_snake[2], (float)pos_snake[0] + 1, (float)pos_snake[1] + 1, (float)pos_snake[2] + 1, 0xff00ff00);
					snake_set.AddSolidColorBox((float)pos_snake[0], 0.0f, (float)pos_snake[2], (float)pos_snake[0] + 1, 0.1f, (float)pos_snake[2] + 1, 0xff000000);
					current = current->getNext();
				}

				roomScene->Models[4] = new Model(&snake_set, XMFLOAT3(0, 0, 0), XMFLOAT4(0, 0, 0, 1),
					new Material(
						new Texture(false, 256, 256, Texture::AUTO_WHITE)
					)
				);
